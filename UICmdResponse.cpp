...
const QString TOKEN_LAT = "lat";
const QString TOKEN_LON = "lon";
const QString TOKEN_ALT = "alt";
...
bool UICmdResponse::fromXML(const QString xml)
{
    ...
    for (int i = 0; i < itemsNode.size(); i++)
    {
        ...
        if (item.hasAttribute(TOKEN_TYPE) && item.hasAttribute(TOKEN_NAME) && item.hasAttribute(TOKEN_DATA))
        {
            ...
            items.append(UICmdResponse::Item(type[0].toLatin1(),
                         item.attribute(TOKEN_NAME),
                         item.attribute(TOKEN_DATA)));
        }
    }
...
QString UICmdResponse::wptoXML() const
{
    QDomDocument doc;
    ...
    for (auto itr = itemswp.begin(); itr != itemswp.end(); itr++)
    {
        QDomElement item = doc.createElement(TOKEN_ITEM);
        item.setAttribute(TOKEN_NAME, itr->name);
        item.setAttribute(TOKEN_LAT, itr->lat);
        item.setAttribute(TOKEN_LON, itr->lon);
        item.setAttribute(TOKEN_ALT, itr->alt);
        response.appendChild(item);
    }
